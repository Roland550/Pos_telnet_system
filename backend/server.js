...
app.post("/deductProduct", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      !updates.every((update) => update.id && update.quantity)
    ) {
    ...
    for (const { id, quantity } of updates) {
      ...
      if (product.quantity < quantity) {
        return res
          .status(400)
          .json({
            message: `Not enough quantity for product ${product.productName}. Available: ${product.quantity}`,
          });
      }
    ...
    res
      .status(500)
      .json({ message: "Error deducting products.", error: error.message });
  }
});
